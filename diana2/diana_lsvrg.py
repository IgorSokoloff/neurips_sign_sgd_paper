...
data_path = '/ibex/scratch/mishchk/diana/data/'
...
# experiment = 'diana_lsvrg_{0}_{1}_{2}_{3}_{4}_{5}'.format(
#     p_norm,
#     alpha,
#     batch,
#     block_size,
...
def generate_update(w, z, X, y, h_i, mu, l2=0, p_norm=2, batch=1, block_size=1):
    ...
    stoch_grad_z = stochastic_logreg_grad(z, X, y, i) + l2 * z
    grad_estimate = mu + stoch_grad_w - stoch_grad_z
...
if rank > 0:
    ...
    while not np.isnan(w).any():
        ...
        if np.isnan(w).any():
            break
        reevaluate_gradients = comm.bcast(None)
        assert reevaluate_gradients in [True, False]
        if reevaluate_gradients:
            mu = l2 * w + logreg_fullgrad(w, X, y)
            z = np.copy(w)
...
if rank == 0:
    ...
    while (it < max_it) and (t < max_t):
        ...
        assert len(w) == d
        comm.Bcast(w)
        reevaluate_gradients = (np.random.uniform(0, 1) < n_workers / N) or (it == 0)
        if reevaluate_gradients:
            it += N
        comm.bcast(reevaluate_gradients)
...
if rank == 0:
    print('There were done', its[-1], 'iterations, which took', ts[-1], 'seconds')
    # step = len(ws) // 200 + 1
